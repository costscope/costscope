...
if ! command -v deadcode >/dev/null 2>&1; then
  echo "deadcode tool not found in PATH – attempting installation (go install github.com/tsenart/deadcode@latest)" >&2
  if command -v go >/dev/null 2>&1; then
    ...
    if GO111MODULE=on GOINSECURE= GONOSUMDB= GONOPROXY= go install github.com/tsenart/deadcode@latest 2>/dev/null; then
      ...
      echo "deadcode tool installed successfully." >&2
    else
      echo "automatic installation failed; please install manually: go install github.com/tsenart/deadcode@latest" >&2
      exit 2
    fi
  else
    echo "Go toolchain not available to install deadcode." >&2
    exit 2
  fi
fi
...
if [[ ! -f "$BASELINE_FILE" ]]; then
  echo "Baseline missing ($BASELINE_FILE). Initialize by copying logs/deadcode_current.json." >&2
...
status=0
if (( count_new > 2 )); then
  echo " Deadcode baseline guard: too many new unreachable symbols ($count_new > 2)." >&2
  status=1
fi
if ((${#missing_rationale[@]})); then
  echo " New symbols without allowlist rationale:" >&2
  printf ' - %s\n' "${missing_rationale[@]}" >&2
  status=1
fi
if [[ -n "$removed_syms" ]]; then
  while IFS= read -r rs; do
    ...
    if grep -E "^$rs" "$ALLOWLIST_FILE" >/dev/null 2>&1; then
      echo " Symbol '$rs' removed from scan but still in allowlist." >&2
      status=1
    fi
  done < <(printf '%s\n' "$removed_syms")
fi
...
if (( status == 0 )); then
  echo " Deadcode baseline guard passed (new=$count_new)."
fi

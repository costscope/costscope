...
// canonicalizeSpec returns a canonical byte representation of the spec content
// to produce a stable hash across environments (e.g., different line endings).
// For YAML/JSON specs, we unmarshal then re-marshal to JSON which has stable key ordering.
func canonicalizeSpec(specPath string, raw []byte) []byte {
	...
	// Fallback to raw bytes if canonicalization fails for any reason
	return raw
}

func toHashBytes(b []byte) string {
	h := sha256.Sum256(b)
...
func main() {
	...
	// Compute a canonical hash from the typed Spec to avoid any environment-specific
	// map key ordering differences when marshaling generic maps.
	// Fall back to raw canonicalization only if marshaling typed struct fails.
	var specHash string
	if jb, err := json.Marshal(s); err == nil {
		specHash = toHashBytes(jb)
	} else {
		specHash = toHashBytes(canonicalizeSpec(*specPath, raw))
	}
